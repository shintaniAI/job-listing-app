...
function stripPlatformBoilerplate(text: string): string {
  let out = text;
  for (const p of PLATFORM_BOILERPLATE_PATTERNS) out = out.replace(p, "");
  return out.replace(/\n{3,}/g, "\n\n").trim();
}
...
async function fetchJinaReader(url: string, timeoutMs = 20000): Promise<string> {
  ...
  try {
    ...
    return stripPlatformBoilerplate(await res.text());
  } catch (e: any) {
...
- **プラットフォーム自身の宣伝文は絶対に使わない**: Wantedly/Talentio/HRMOS/Herp/Indeed/doda/マイナビ/リクナビ/エン 等が自社を紹介する文言（例:「Wantedlyは〇〇万人のユーザー...のビジネスSNS」「共感を軸にした新しい挑戦」「このサイトは採用管理システム『...』で作成」等）は対象企業とは無関係のため出力に含めない
...
# workConditions（勤務条件）
  推奨キー: 勤務地 / 勤務地住所 / 最寄り駅 / 勤務時間 / フレックス / コアタイム / 清算期間 / 休憩時間 / リモートワーク / 残業 / 月平均残業時間 / 有給取得率 / 試用期間 / 転勤 / 副業 / 服装
# selection（選考プロセス）※ 任意
  推奨キー: 応募方法 / 選考フロー / 面接回数 / カジュアル面談 / 必要書類 / 内定までの期間 / 応募後の流れ
