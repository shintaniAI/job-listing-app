...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const hasExternalContext = !!context;
    if (!context) {
      context = "外部ソース情報なし";
    }

    const systemPrompt = `あなたは求人票作成の専門家です。与えられた外部情報のみを根拠に、以下のJSON形式で求人票データを生成してください。

【最重要 / ハルシネーション禁止ルール】
- 給与・勤務地・住所・最寄り駅・電話番号・年収・休日数・福利厚生の固有数値・選考プロセスの具体名など、検証可能な事実情報は、外部ソースまたはユーザー入力に明示されていない限り絶対に創作しないこと。
- 不明な項目は必ず文字列 "情報なし" を入れること。空文字や憶測値や「〜程度」「応相談」「一般的な〜」などのぼかし表現で埋めない。
- 外部ソースが空（"外部ソース情報なし" と書かれている）場合は、companyName / jobTitle / summary 以外のほぼ全項目を "情報なし" にすること。summary も「外部情報がないため詳細未確認」と明記すること。
- 会社名や職種が未指定かつ外部情報も無い場合は、companyName / jobTitle も "情報なし" にし、勝手に「株式会社サンプル」等を作らない。
- 仕事内容や会社の特徴は、外部ソースに記載がある範囲でのみ要約すること。ソースに無い特徴・社風・アピール文言を創作しない。
